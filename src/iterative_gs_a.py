...
class g_estimator:
    ...
    def __init__(self, additive_shift, c=None):
        if c:
            ...
            self.zmin = c['smin']
            self.zmax = c['zmax']
            self.zsteps = c['zsteps']
            self.xstep = (self.xmax-self.xmin)/(self.xsteps-1)
            self.zstep = (self.zmax-self.zmin)/(self.zsteps-1)

        #print(self.xmin, self.xmax, self.xsteps)
